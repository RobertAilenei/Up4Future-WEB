    ...
    public void add(Startup startup) throws RepositoryException {
        ...
        try {
            try (PreparedStatement statement = conn.prepareStatement("INSERT INTO Startup VALUES (?, ?, ?, ?)")) {
                ...
                statement.setString(3,startup.getStartupEmail());
                statement.setString(4,startup.getStartupPassword());
    ...
    private void getAll() {
        ...
        try {
            try (PreparedStatement statement = conn.prepareStatement("SELECT * from Startup");
                ...
                while (rs.next()) {
                    Startup startup = new Startup(rs.getInt("id"), rs.getString("name"), rs.getString("email"), rs.getString("password"));
